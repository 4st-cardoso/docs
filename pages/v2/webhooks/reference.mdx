...
|------------------------------------------------------|--------------------|
| `POST /v2/orders`, `POST /v2/subscriptions` e as faturas dessas assinaturas | v2 |
| Rotas da API v1                                      | v1                 |

O mesmo endpoint pode receber os dois formatos. Use o campo `webhook_version` para decidir como ler o payload.

<Note>
  Eventos que não existem no formato v2 são entregues no formato v1, mesmo para recursos criados pela v2. É o caso de `order.challenged`, `charge.challenged` e dos eventos de cartão (`card.*`).
</Note>
...
<Info>
  A verificação por código (OTP) não gera webhook no formato v2. Ela gera `order.challenged` no formato v1, inclusive a cada reenvio do código, e aparece no `next_action` da resposta do pedido ou da fatura.
</Info>
