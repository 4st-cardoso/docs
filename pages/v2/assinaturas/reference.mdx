...
| `PUT` | `/v2/subscriptions/{id}/payment-method`     | [Altera o meio de pagamento](/pages/v2/assinaturas/payment-method) |
| `PATCH` | `/v2/subscriptions/{id}/items/{code}`       | [Atualiza um item](/pages/v2/assinaturas/update-item) |
| `PUT` | `/v2/subscriptions/{id}/next-billing-date`  | [Altera a data da próxima cobrança](/pages/v2/assinaturas/next-billing-date) |
...
| `currency`             | string  | Não  | `BRL` ou `USD` (padrão `BRL`). Moeda em que o preço da assinatura é acordado. Na resposta, vem em minúsculas |
| `international`        | boolean | Não  | Restringe a escolha às adquirentes habilitadas para compra internacional (padrão `false`). Não tem relação com `currency` e não é devolvido na resposta |
...
Quando o split está habilitado na plataforma, as regras abaixo são validadas na criação, e não só na primeira cobrança. Com o split desabilitado, elas não são verificadas:
...
{
  "split": [
    { "receiver_code": "re_4kT9pWq2Lx7ZbN1mRv5c", "amount": 8000, "options": { "charge_processing_fee": true, "charge_remainder_fee": true, "liable": true } },
    { "receiver_code": "re_Hs8dF3jK6aQ0wE2yU9tP", "amount": 2000 }
  ]
}
```
...
<Note>
  A assinatura é um contrato de preço, não uma cobrança: o preço foi acordado em uma moeda e não muda porque o câmbio mudou. Por isso `amount`, `items`, `discounts` e `increments` da assinatura ficam na moeda de `currency`. O valor cobrado de cada ciclo está nas [faturas](/pages/v2/faturas/reference), que respondem em BRL, com `original_amount`, `original_currency` e `exchange_rate`.
</Note>
...
| `currency`             | string  | Moeda em que o preço foi acordado, em minúsculas (`brl`, `usd`). **Não** é convertida para real |
