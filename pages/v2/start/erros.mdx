...
| `code`                           | Quando acontece                                                   |
|----------------------------------|-------------------------------------------------------------------|
| `api_rate_limited`               | Limite geral da chave de API ou da conta (120 por minuto)         |
...
<Warning>
  O `type` de um `429` é `invalid_request_error`, e não um tipo próprio de limite. **Identifique o estrangulamento pelo `code`** (`api_rate_limited`, `rate_limit_exceeded` ou `card_tokenization_rate_limited`), nunca pelo `type`.
</Warning>
...
|--------|---------------------------------------------------------------------------------------|----------------------------------------|
| `429`  | Limite de reenvio do código de verificação (5 por minuto por IP)                       | `{ "message": "Too Many Attempts." }` |
