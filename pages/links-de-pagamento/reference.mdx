...
Os dados do cliente são pré-preenchidos no checkout. O cliente pode editar as informações antes de pagar.
...
| `client.document.type`   | string | Não  | `CPF`, `CNPJ`, `passport`, `undefined`                 |
| `client.document.number` | string | Não  | Número do documento (CPF/CNPJ validado com dígito)     |
