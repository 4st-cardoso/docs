...
| `webhook_version` | string | Versão do formato do payload — sempre `v1` nesta documentação |
| `data`            | object | Dados específicos do evento (veja cada evento abaixo)         |

<Note>
  O campo `webhook_version` acompanha **todos** os eventos desta documentação com o valor `v1`. Ele identifica o formato do payload, para que o seu endpoint possa conviver com formatos futuros sem ambiguidade. Trate valores desconhecidos de forma tolerante.
</Note>
...
| `tax_invoice.issued`         | A nota fiscal de um pedido foi emitida e autorizada         |

Os payloads detalhados de cada evento estão na seção **Eventos** na barra lateral.

<Note>
  A tabela acima lista os eventos cujos payloads estão documentados nesta versão. A lista completa dos eventos que podem ser assinados — incluindo recebedores, KYC e saques — está em [Endpoints de Webhook → Eventos disponíveis](/pages/webhook-endpoints/reference#eventos-disponíveis).
</Note>
