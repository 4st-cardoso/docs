...
description: "Como gerar, usar e revogar a chave que autentica na API v2 e na API de Análise de Fraude."
icon: "key"
---

<Warning>
  A **chave de API (secret key)** autentica **todas as rotas da API v2** e as **rotas de Análise de Fraude** da v1.
  As demais rotas da v1 continuam usando o [token de login](/pages/autenticacao/reference) (`Authorization: Bearer <token JWT>`).
</Warning>

## O que é

A **chave de API** (secret key) é a credencial que autentica as chamadas à [API v2](/pages/v2/start/autenticacao) e à API de Análise de Fraude. Ela pertence à sua conta e vai no header `Authorization` de cada requisição:
